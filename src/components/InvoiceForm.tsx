...
export function InvoiceForm({ initialData }: InvoiceFormProps) {
  ...
  const { addInvoice, updateInvoice, invoices, getInvoiceById } = useInvoices();
  ...
  const onSubmit = async (data: InvoiceFormValues) => {
    ...
    if (isEditMode && initialData) {
        const existingInvoice = getInvoiceById(initialData.id);
        result = await updateInvoiceAction({ ...data, id: initialData.id, invoiceNumber: existingInvoice?.invoiceNumber || "" });
    } else {
        const existingInvoiceNumbers = invoices.map((jo) => jo.invoiceNumber);
  ...
  const handlePrint = () => {
    if (lastSavedInvoice) {
        window.open(`/print/${lastSavedInvoice.id}`, '_blank');
    }
    handleDialogClose();
  }
          ...
          <div className="grid md:grid-cols-2 gap-8">
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Add any extra notes or instructions for this invoice.</CardDescription>
                </CardHeader>
                <CardContent>
                    <FormField
                        control={form.control}
